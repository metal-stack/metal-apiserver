	...
	tests := []struct {
		...
		},
		{
			...
			preparefn: func(t *testing.T) {
				test.CreateNetworks(t, testStore, []*adminv2.NetworkServiceCreateRequest{
					{
						Id:                       new("tenant-super-network-namespaced"),
						Prefixes:                 []string{"10.100.0.0/14"},
						DefaultChildPrefixLength: &apiv2.ChildPrefixLength{Ipv4: new(uint32(22))},
		...
		},
		{
			...
			preparefn: func(t *testing.T) {
				test.CreateNetworks(t, testStore, []*adminv2.NetworkServiceCreateRequest{
					{
						Id:                       new("tenant-super-with-partition"),
						Prefixes:                 []string{"10.100.0.0/14"},
						DefaultChildPrefixLength: &apiv2.ChildPrefixLength{Ipv4: new(uint32(22))},
						Type:                     apiv2.NetworkType_NETWORK_TYPE_SUPER,
						Partition:                new("partition-one"),
		...
		},
		{
			...
			preparefn: func(t *testing.T) {
				test.CreateNetworks(t, testStore, []*adminv2.NetworkServiceCreateRequest{
					{
						Id:                       new("project-scoped-tenant-super"),
						Prefixes:                 []string{"10.100.0.0/14"},
						DestinationPrefixes:      []string{"1.2.3.0/24"},
						DefaultChildPrefixLength: &apiv2.ChildPrefixLength{Ipv4: new(uint32(22))},
						Project:                  new(p1),
		...
		},
		{
			...
			want:    nil,
			wantErr: errorutil.InvalidArgument("not allowed to create child network with project %s in network project-scoped-tenant-super scoped to project %s", p2, p1),
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			preparefn: func(t *testing.T) {
				test.CreateNetworks(t, testStore, []*adminv2.NetworkServiceCreateRequest{
					{
						Id:                       new("tenant-super-namespaced-1"),
	...
	tests := []struct {
		...
		},
		{
			...
			rq: &adminv2.NetworkServiceCreateRequest{
				Id:                       new("tenant-super-network"),
				Prefixes:                 []string{"10.100.0.0/24"},
				DefaultChildPrefixLength: &apiv2.ChildPrefixLength{Ipv4: new(uint32(26))},
		...
		},
		{
			...
			rq: &adminv2.NetworkServiceCreateRequest{
				...
				Prefixes:                   []string{"10.100.0.0/24"},
				DefaultChildPrefixLength:   &apiv2.ChildPrefixLength{Ipv4: new(uint32(26))},
				MinChildPrefixLength:       &apiv2.ChildPrefixLength{Ipv4: new(uint32(25))},
...
func Test_networkServiceServer_CreateExternal(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			rq: &adminv2.NetworkServiceCreateRequest{
				Id:       new("project-scoped-internet-3"),
...
func Test_networkServiceServer_Delete(t *testing.T) {
	...
	networkMap := test.AllocateNetworks(t, testStore, []*apiv2.NetworkServiceCreateRequest{
		{Name: new("tenant-1"), Project: p1, Partition: new("partition-one")},
		{Name: new("tenant-2"), Project: p1, Partition: new("partition-one")},
	})

	test.CreateIPs(t, testStore, []*apiv2.IPServiceCreateRequest{
		{Network: networkMap["tenant-1"].Id, Project: p1, Name: new("ip-1")},
	})
...
func Test_networkServiceServer_List(t *testing.T) {
	...
	networkMap := test.AllocateNetworks(t, testStore, []*apiv2.NetworkServiceCreateRequest{
		{Name: new("tenant-1"), Project: p1, Partition: new("partition-one")},
		{Name: new("tenant-2"), Project: p1, Partition: new("partition-one"), Labels: &apiv2.Labels{Labels: map[string]string{"size": "small", "color": "blue"}}},
	})
	...
	tests := []struct {
		...
		// },
		{
			name: "specific id",
			rq: &adminv2.NetworkServiceListRequest{
				Query: &apiv2.NetworkQuery{Id: new(networkMap["tenant-1"].Id)},
			},
			want: &adminv2.NetworkServiceListResponse{
				Networks: []*apiv2.Network{
					{
		...
		},
		{
			...
			rq: &adminv2.NetworkServiceListRequest{
				Query: &apiv2.NetworkQuery{Partition: new("partition-one"), Type: apiv2.NetworkType_NETWORK_TYPE_SUPER.Enum()},
			},
			want: &adminv2.NetworkServiceListResponse{
				Networks: []*apiv2.Network{
					{
