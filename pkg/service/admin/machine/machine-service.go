...
func (m *machineServiceServer) BMCCommand(ctx context.Context, req *adminv2.MachineServiceBMCCommandRequest) (*adminv2.MachineServiceBMCCommandResponse, error) {
	...
	taskID, err := m.repo.UnscopedMachine().AdditionalMethods().MachineBMCCommand(ctx, machine.Uuid, machine.Partition.Id, req.Command)
	if err != nil {
		return nil, err
	}

	return &adminv2.MachineServiceBMCCommandResponse{
		TaskId: taskID,
	}, nil
}
