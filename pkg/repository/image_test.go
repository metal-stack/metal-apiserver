...
func Test_sortImages(t *testing.T) {
	...
	for i := range tests {
		tt := tests[i]
		t.Run(tt.name, func(t *testing.T) {
			if got := is.SortImages(tt.images); !reflect.DeepEqual(got, tt.want) {
...
func TestGetOsAndSemver(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			os:      "ubuntu",
			version: semver.MustParse("19.4.20200408"),
			wantErr: false,
		},
		{
			name:    "twoparts",
			id:      "ubuntu-small-19.4.20200408",
			os:      "ubuntu-small",
			version: semver.MustParse("19.4.20200408"),
			wantErr: false,
		},
		{
			name:    "fourparts",
			id:      "ubuntu-is-very-small-19.4.20200408",
		...
		{
			name:    "firewall",
			id:      "firewall-2.4.20200408",
			os:      "firewall",
			version: semver.MustParse("2.4.20200408"),
			wantErr: false,
		},
		{
			name:    "firewall-ubuntu",
			id:      "firewall-ubuntu-2.4.20200408",
			os:      "firewall-ubuntu",
			version: semver.MustParse("2.4.20200408"),
			wantErr: false,
		},
