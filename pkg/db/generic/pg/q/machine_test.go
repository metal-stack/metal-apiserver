...
var (
	pgM1 = &metal.Machine{
		ID: "6b22ccd6-4c93-4a1f-8c8f-2f63b3c6e001", Name: "m1",
	...
	pgM2 = &metal.Machine{
		ID: "6b22ccd6-4c93-4a1f-8c8f-2f63b3c6e002", Name: "m2",
		Allocation: &metal.MachineAllocation{
			Creator:     "",
			Created:     time.Time{},
			Name:        "shoot-fw-m2",
	...
	pgM3 = &metal.Machine{
		ID: "6b22ccd6-4c93-4a1f-8c8f-2f63b3c6e003", Name: "m3",
		Allocation: &metal.MachineAllocation{
			Creator:     "",
			Created:     time.Time{},
			Name:        "",
	...
	pgM4 = &metal.Machine{
		ID: "6b22ccd6-4c93-4a1f-8c8f-2f63b3c6e004", Name: "m4",
	...
	pgMachines = []*metal.Machine{pgM1, pgM2, pgM3, pgM4}
)

// TestMachineFilterPostgres mirrors the RethinkDB-backed TestMachineFilter from
// pkg/db/queries/machine_test.go, but runs against the Postgres backend using
// the pg.GenericRepository and querypg.MachineFilter.
func TestMachineFilterPostgres(t *testing.T) {
	log := slog.New(slog.NewJSONHandler(os.Stdout, &slog.HandlerOptions{Level: slog.LevelDebug}))
	db, closer := test.StartPostgres(t, log)
	defer closer()

	repo, err := pg.NewGenericRepository[*metal.Machine](log, db)
	...
	list := func(rq *apiv2.MachineQuery) []*metal.Machine {
		t.Helper()
		ents, err := repo.Query(ctx, q.MachineFilter(rq), nil)
		require.NoError(t, err)
		got := make([]*metal.Machine, 0, len(ents))
		for _, e := range ents {
	...
	tests := []struct {
	...
	}{
		{
			...
			rq:   nil,
			want: []*metal.Machine{pgM1, pgM2, pgM3, pgM4},
		},
		{
			name: "by id",
			rq:   &apiv2.MachineQuery{Uuid: &pgM1.ID},
			want: []*metal.Machine{pgM1},
		},
		{
			name: "by id 2",
			rq:   &apiv2.MachineQuery{Uuid: &pgM2.ID},
			want: []*metal.Machine{pgM2},
		},
		{
		...
		{
			name: "by name",
			rq:   &apiv2.MachineQuery{Name: &pgM1.Name},
			want: []*metal.Machine{pgM1},
		},
		{
			name: "by partition",
			rq:   &apiv2.MachineQuery{Partition: &pgM1.PartitionID},
			want: []*metal.Machine{pgM1},
		},
		{
			name: "by size",
		...
		},
		{
			...
			want: []*metal.Machine{pgM1, pgM2, pgM3},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(
				...
				t.Errorf("machine list = %v, want %v diff: %s", got, tt.want, diff)
			}
		})
	}
}
...
func TestMachineFilter(t *testing.T) {
	...
	filters := q.MachineFilter(&apiv2.MachineQuery{
		Partition:    &partition,
		Size:         &size,
		Rack:         &rack,
		Room:         &room,
...
func TestMachineFilterNilQuery(t *testing.T) {
	require.Nil(t, q.MachineFilter(nil))
}
